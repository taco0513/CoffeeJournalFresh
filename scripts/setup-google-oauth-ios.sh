...
# Setup Google OAuth for iOS - Coffee Journal Fresh
...
if [ ! -f "ios/CoffeeJournalFresh/Info.plist" ]; then
    ...
    echo "Expected to find: ios/CoffeeJournalFresh/Info.plist"
    exit 1
fi
...
echo "   - Open ios/CoffeeJournalFresh.xcworkspace in Xcode"
echo "   - Select CoffeeJournalFresh target"
