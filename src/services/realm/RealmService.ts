...
class RealmService {
  ...
  // Coffee Comparison Functions
  // 같은 커피 조합 검색 (coffee_name + roaster_name)
  getSameCoffeeComparison(coffeeName: string, roastery: string): {
    totalTastings: number;
    averageScore: number;
    popularFlavors: { value: string; count: number; percentage: number }[];
    sensoryAverages: {
      body: number;
      acidity: number;
      sweetness: number;
    ...
    try {
      ...
      if (sameCoffeeRecords.length === 0) {
        return {
          ...
          averageScore: 0,
          popularFlavors: [],
          sensoryAverages: { body: 0, acidity: 0, sweetness: 0, finish: 0 }
        };
      }
      
      // 평균 점수 계산
      const totalScore = sameCoffeeRecords.reduce((sum, record) => sum + record.matchScoreTotal, 0);
      const averageScore = Math.round(totalScore / sameCoffeeRecords.length);
      
      // 맛 노트 빈도 계산
      const flavorCounts = new Map<string, number>();
      sameCoffeeRecords.forEach(record => {
        record.flavorNotes.forEach(note => {
          if (note.level === 1) { // Level 1 flavors만 카운트
            flavorCounts.set(note.value, (flavorCounts.get(note.value) || 0) + 1);
          }
        });
      });
      ...
      // 인기 맛 노트 정렬
      const popularFlavors = Array.from(flavorCounts.entries())
        .map(([value, count]) => ({
          value,
          count,
          percentage: Math.round((count / sameCoffeeRecords.length) * 100)
        }))
        .sort((a, b) => b.count - a.count)
        .slice(0, 5); // 상위 5개
      ...
      const sensoryAverages = {
        body: Math.round(sameCoffeeRecords.reduce((sum, record) => sum + (record.sensoryAttribute?.body || 3), 0) / sameCoffeeRecords.length),
        acidity: Math.round(sameCoffeeRecords.reduce((sum, record) => sum + (record.sensoryAttribute?.acidity || 3), 0) / sameCoffeeRecords.length),
        sweetness: Math.round(sameCoffeeRecords.reduce((sum, record) => sum + (record.sensoryAttribute?.sweetness || 3), 0) / sameCoffeeRecords.length),
        finish: Math.round(sameCoffeeRecords.reduce((sum, record) => sum + (record.sensoryAttribute?.finish || 3), 0) / sameCoffeeRecords.length)
      };
      
      return {
        totalTastings: sameCoffeeRecords.length,
        averageScore,
        popularFlavors,
        sensoryAverages
      };
  ...
  // 비슷한 커피 찾기 (같은 origin이나 roastery)
  getSimilarCoffees(coffeeName: string, roastery: string, origin?: string): Array<{
    coffeeName: string;
    roastery: string;
    origin: string;
    averageScore: number;
    ...
    try {
      ...
      if (origin) {
        query += ' AND origin = $0 AND NOT (coffeeName = $1 AND roastery = $2)';
        params.push(origin, coffeeName, roastery);
      } else {
        query += ' AND roastery = $0 AND NOT coffeeName = $1';
        params.push(roastery, coffeeName);
      }
      
      const similarRecords = realm.objects<ITastingRecord>('TastingRecord')
        .filtered(query, ...params);
      
      // 커피별로 그룹화
      const coffeeGroups = new Map<string, {
        coffeeName: string;
        roastery: string;
        origin: string;
        scores: number[];
      ...
      similarRecords.forEach(record => {
        const key = `${record.coffeeName}::${record.roastery}`;
        if (!coffeeGroups.has(key)) {
          coffeeGroups.set(key, {
            coffeeName: record.coffeeName,
            roastery: record.roastery,
            origin: record.origin || '',
            scores: []
        ...
        coffeeGroups.get(key)!.scores.push(record.matchScoreTotal);
      });
      
      // 평균 점수 계산 후 정렬
      return Array.from(coffeeGroups.values())
        .map(group => ({
          coffeeName: group.coffeeName,
          roastery: group.roastery,
          origin: group.origin,
          averageScore: Math.round(group.scores.reduce((sum, score) => sum + score, 0) / group.scores.length),
          tastingCount: group.scores.length
        }))
