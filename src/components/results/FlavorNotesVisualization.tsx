...
interface FlavorNotesVisualizationProps {
  flavorPaths: unknown[]; // FlavorPath array
  selectedExpressions?: unknown[]; // Korean sensory expressions
  title?: string;
...
export const FlavorNotesVisualization: React.FC<FlavorNotesVisualizationProps> = ({
  ...
  const groupedFlavors = flavorPaths.reduce((acc: unknown, path: unknown) => {
  ...
  const formatFlavorPath = (path: unknown): string => {
    const parts = [];
  ...
  const groupedExpressions = selectedExpressions.reduce((acc: unknown, expr: unknown) => {
          ...
          {showHierarchy ? (
              ...
              {Object.entries(groupedFlavors).map(([category, flavors]: [string, any]) => (
                <View key={category} style={styles.categoryGroup}>
                  ...
                  <View style={styles.flavorList}>
                    {flavors.map((flavor: unknown, index: number) => (
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={styles.chipContainer}>
                {flavorPaths.map((path: unknown, index: number) => (
            ...
            {Object.entries(groupedExpressions).map(([category, expressions]: [string, any]) => (
              <View key={category} style={styles.expressionGroup}>
                ...
                <View style={styles.expressionChips}>
                  {expressions.map((expr: unknown, index: number) => (
