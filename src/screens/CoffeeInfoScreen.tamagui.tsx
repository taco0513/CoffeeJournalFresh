...
const CoffeeInfoScreenTamagui = () => {
  ...
  const loadSuggestions = async () => {
    try {
      ...
      // 고유값 추출
      const cafes = [...new Set(tastingArray.map(t => t.cafeOrRoaster).filter(Boolean))];
      const roasters = [...new Set(tastingArray.map(t => t.roaster).filter(Boolean))];
      const coffeeNames = [...new Set(tastingArray.map(t => t.coffeeName).filter(Boolean))];
      const origins = [...new Set(tastingArray.map(t => t.origin).filter(Boolean))];
      const varieties = [...new Set(tastingArray.map(t => t.variety).filter(Boolean))];
      const processes = [...new Set(tastingArray.map(t => t.process).filter(Boolean))];
  ...
  // 필수 필드 검증
  const isValid = currentTasting.cafeOrRoaster?.trim() && currentTasting.coffeeName?.trim();
            ...
            <FormCard elevate size="$4" animation="lazy">
              <YStack space="$md">
                ...
                <YStack>
                  ...
                  <AutocompleteInput
                    value={currentTasting.cafeOrRoaster || ''}
                    onChangeText={(value) => updateField('cafeOrRoaster', value)}
