...
export class BiometricAuth {
  ...
  static async initialize(): Promise<void> {
    try {
      ...
      console.log('BiometricAuth initialized:', this.capabilities);
    } catch (error) {
   ...
   */
  private static handleBiometricError(error: any): BiometricResult {
    console.error('Biometric authentication error details:', error);
   ...
   */
  private static isNotEnrolledError(error: any): boolean {
    const errorCode = error.code;
   ...
   */
  private static getCapabilityErrorMessage(error: any): string {
    const errorCode = error.code;
    const errorMessage = error.message?.toLowerCase() || '';
